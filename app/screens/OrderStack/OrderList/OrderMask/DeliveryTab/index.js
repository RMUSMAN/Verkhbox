...
const DeliveryTab = (props) => {
  ...
  const [bookID, setBookId] = useState("Booking ID 01233");
  ...
  useEffect(() => {
    ...
    dispositions =
      dispositions?.length > 0
        ? dispositions.filter((elem) => elem.type === "plan_delivery")
        : [];

    let tableDataCopy = dispositions.map((elem) => [
  ...
  const handleSave = () => {
    ...
    const addList =
      tableData?.length > 0
        ? tableData
            .map((elem) => {
              if (!elem[2] || parseInt(elem[2]) <= 0) {
                return;
              }
              let objValue = {
                product_id: elem[4].id,
                plan_qty: parseInt(elem[2]),
        ...
        <Button
          ...
          onPress={() => {
            ...
            const productValue = {
              ...
              order_id: taskInfo.order_id,
              tableData,
              type: StringsOfLanguages.delivered,
              signDocs: [],
              orderDocs: [],
            };
